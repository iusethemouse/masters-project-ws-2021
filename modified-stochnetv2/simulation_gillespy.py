...
def _single_simulation(
    ...
    if model_name != 'BD20':
        list_of_colours = [x for x in colours.ColorConverter.colors.keys() if len(x)<3]*10
    else:
        list_of_colours = ["#"+''.join([random.choice('0123456789ABCDEF') for j in range(6)]) for i in range(20)]
    ...
    plt.savefig(fr"{model_name}\{crn_instance.get_n_species()}_species_{fname}.png")
    plt.close()
