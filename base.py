...
class BaseCRNModel(gillespy.Model):
    ...
    def get_randomized_parameters(cls, param_names, n_settings, sigm=0.1):
        ...
        limit = math.floor(n_settings / len(param_names))
        counter = 0
        current_param_idx = 0
        for s in range(n_settings):
            if (counter >= limit) and not (current_param_idx == len(param_names) - 1):
                current_param_idx += 1
                counter = 0
            for i, param in enumerate(param_names):
                default = float(cls.params[param])
                if i == current_param_idx:
                    randomized[param].append(np.random.uniform(default * (1 - sigm), default * (1 + sigm), 1))
                else:
                    randomized[param].append(np.asarray([default]))
            counter += 1

        for param in param_names:
            randomized[param] = np.squeeze(np.array(randomized[param]))
